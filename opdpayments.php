...
<body>
            ...
            <div class="sb2-2">
                ...
                <div class="sb2-2-2">
                    ...
                    <li><a href="admin.php"><i class="fa fa-home" aria-hidden="true"></i> Home</a>
                        ...
                        <li class="active-bre"><a href="#"> OPD Payment History</a>
                        </li>
  ...
  include ('phpquery/dbconnection.php');
   ...
   $stmt = mysqli_prepare($con,"SELECT * FROM opd_payments as o INNER JOIN doctor_specialist AS d ON o.user_id=d.user_id AND o.p_id='".$_SESSION['p_id']."' ");
   ...
   while($row = $result->fetch_assoc()) {
    ...
    $apt_id= $row['apt_id'];
    $charge=$row['charge']; 
    $payment_date=   date('dS F Y', strtotime($row['apt_date']));
    $date=   date('dS F Y', strtotime($row['date']));

 ?>
                                                <tr>
                                                <td><?php echo  $row['opd_payment_id']   ?></td>
													<td><?php echo $apt_id   ?></td>
                                                 ...
                                                 <?php    } elseif($row['status']=='2') { ?>
                                                    ...
                                                    <?php    } elseif($row['status']=='4') { ?>
                                                        <td> <span class="label label-primary">No need</span> </td>
